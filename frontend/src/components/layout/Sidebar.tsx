...
// Shared items mirror the mobile bottom nav (same arcade icons + labels).
// "Alerts" is desktop-only (no mobile equivalent) but uses a matching arcade icon.
const NAV_ITEMS = [
  ...
  { icon: AlertNavIcon, label: "Alerts", href: "/notifications" },
] as const;
    ...
    <div className="tournament-sidebar game-panel flex h-full flex-col rounded-[26px] border border-white/[0.08] p-2 text-white shadow-[0_22px_70px_rgba(9,12,30,.18)] xl:p-4">
      ...
      <nav className="flex-1 space-y-1.5 border-t border-white/[0.07] pt-4">
        {NAV_ITEMS.map((item) => {
          ...
          const isAuthRequired = item.href === "/profile";
          return (
            <Link
              key={item.label}
              href={href}
            ...
            >
              ...
              >
                <div className="relative flex items-center justify-center shrink-0">
                  <item.icon className="h-8 w-9" active={isActive} />
